...
AgainDeleteSession:
IF @isShared=1 BEGIN
	EXEC @resultSession=sp_releaseapplock @Resource=@Name, @LockOwner='Session'
END ELSE BEGIN
	EXEC @resultSession=tempdb..sp_releaseapplock @Resource=@Name, @LockOwner='Session'
END
...
IF @isShared=1 BEGIN
	EXEC @Result=sp_getapplock @Resource=@Name, @LockOwner='Session', @LockMode = 'exclusive', @LockTimeOut=@Timeout
END ELSE BEGIN
	EXEC @Result=tempdb..sp_getapplock @Resource=@Name, @LockOwner='Session', @LockMode = 'exclusive', @LockTimeOut=@Timeout
END
RETURN @Result
END TRY
BEGIN CATCH
